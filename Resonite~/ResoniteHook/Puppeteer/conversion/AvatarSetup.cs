...
public partial class RootConverter
{
    ...
    {
        // Clear all autogenerated viseme drivers (these use a bit of an over-eager heuristic; since we have
        // actual data on what we should be using for visemes, there's no need to use these heuristics)
        foreach (var autodriver in _root.GetComponentsInChildren<f.DirectVisemeDriver>())
        {
            autodriver.Destroy();
        }
